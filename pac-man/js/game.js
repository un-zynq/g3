...
function gameover() { 
	GAMEOVER = true;
	message(IS_MOBILE ? "tap movement buttons to return" : "game over press any key to return");
	stopTimes();
...
function message(m) { 
	$("#message").html(m);
	if (m === "game over press any key to return" || m === "tap movement buttons to return") $("#message").addClass("red");
}
